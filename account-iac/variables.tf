# Variables for account infrastructure
...
variable "account_id" {
  description = "Unique account identifier (defaults to workspace prefix if not provided)"
  type        = string
  default     = ""
  
  validation {
    condition     = var.account_id == "" || can(regex("^[a-zA-Z0-9-_]+$", var.account_id))
    error_message = "Account ID can only contain alphanumeric characters, hyphens, and underscores."
  }
}
...
variable "account_aws_profile" {
  description = "AWS profile for account account authentication (only for local development, CI/CD uses environment variables)"
  type        = string
  default     = "default"
}
