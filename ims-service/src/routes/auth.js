    ...
    try {
        ...
        try {
            ...
            if (accountId) {
                ...
                const rbacUsers = await rbacService.getAllUsersInAccount(
                    accountId,
                );
                const rbacUser = rbacUsers.find(
                ...
                if (rbacUser) {
                    ...
                    logger.info('Updated user password status in RBAC', {
                        username,
                        userId: rbacUser.user_id,
                    });
                }
            }
        } catch (rbacError) {
            logger.warn(
                'Failed to update RBAC user status, but password change succeeded',
                {
                    username,
        ...
        logger.info('Password changed successfully, user logged in', {
            username,
        });
        ...
        logger.error('Complete new password challenge failed', {
            error: error.message,
        });
...
/**
 * POST /auth/complete-password-challenge
 * Alias for complete-new-password-challenge for frontend compatibility
 * This ensures both endpoint names work without breaking existing functionality
 */
router.post('/complete-password-challenge', async (req, res) => {
    try {
        const {username, newPassword, session} = req.body;
        ...
        logger.info('Completing password challenge (alias endpoint)', {
            username,
        });
        ...
        logger.info(
            'Password changed successfully via alias endpoint, user logged in',
            {username},
        );
        ...
        logger.error('Complete password challenge failed (alias endpoint)', {
            error: error.message,
        });
    ...
    try {
        ...
        if (
            ...
            return res.status(400).json({
                error: 'Bad Request',
                message:
                    'Username, email, password, first name, last name, and account ID are required',
            });
        }
    ...
    try {
        ...
        if (!username || !newPassword || !session || !challengeName) {
            return res.status(400).json({
                error: 'Bad Request',
                message:
                    'Username, newPassword, session, and challengeName are required',
            });
        }
