# Create Infrastructure Worker Lambda Function
# Handles account infrastructure creation
...
locals {
  function_name = "${var.project_name}-${var.environment}-create-infra-worker"
  
  common_tags = merge(var.common_tags, {
    Name    = local.function_name
    Purpose = "AccountInfrastructureCreation"
    Type    = "Lambda"
    Component = "Worker"
  })
}
...
# Lambda Function
resource "aws_lambda_function" "create_infra_worker" {
  ...
  filename         = "${path.root}/lambda-packages/create-infra-worker.zip"
  source_code_hash = filebase64sha256("${path.root}/lambda-packages/create-infra-worker.zip")
...
# Custom IAM policy for create infra operations
