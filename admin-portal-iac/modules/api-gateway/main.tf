...
# CONSOLIDATED: Using broad wildcards to stay within AWS 20KB policy limit
# All /api/v1/* routes are covered by these permissions
# ==============================================

# Permission 1: /api/v1/* (covers ALL API v1 routes including /app/*, /app/api/*, etc.)
resource "aws_lambda_permission" "app_backend_v1_all" {
  count         = var.app_backend_lambda_function_name != "" ? 1 : 0
  statement_id  = "AllowAPIGateway-V1All"
  action        = "lambda:InvokeFunction"
  function_name = var.app_backend_lambda_function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.admin_api.execution_arn}/*/*/api/v1/*"
}

# Permission 2: /api/v1 base path (for requests without trailing path)
resource "aws_lambda_permission" "app_backend_v1_base" {
  count         = var.app_backend_lambda_function_name != "" ? 1 : 0
  statement_id  = "AllowAPIGateway-V1Base"
  action        = "lambda:InvokeFunction"
  function_name = var.app_backend_lambda_function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.admin_api.execution_arn}/*/*/api/v1"
