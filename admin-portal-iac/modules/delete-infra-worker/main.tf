# Delete Infrastructure Worker Lambda Function
# Handles account infrastructure deletion
...
locals {
  function_name = "${var.project_name}-${var.environment}-delete-infra-worker"
  
  common_tags = merge(var.common_tags, {
    Name    = local.function_name
    Purpose = "AccountInfrastructureDeletion"
    Type    = "Lambda"
    Component = "Worker"
  })
}
...
# Lambda Function
resource "aws_lambda_function" "delete_infra_worker" {
  ...
  filename         = "${path.root}/lambda-packages/delete-infra-worker.zip"
  source_code_hash = filebase64sha256("${path.root}/lambda-packages/delete-infra-worker.zip")
...
# Custom IAM policy for delete infra operations
