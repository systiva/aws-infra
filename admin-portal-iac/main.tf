...
# Retrieve Account Registry Table Name from SSM (created by admin bootstrap)
data "aws_ssm_parameter" "account_registry_table_name" {
  name = "/admin-portal/${var.workspace_prefix}/admin/bootstrap/account-registry-table"
}
...
locals {
  ...
  cross_account_role_name = "${local.workspace_prefix}-CrossAccountAccountRole"

  # DynamoDB table naming - uses naming convention instead of SSM parameter
  # SSM parameter is stored in account account (cross-account access not supported)
  # Convention: account-admin-public-{workspace}
  account_public_table_name = "account-admin-public-${local.workspace_prefix}"

  # Step Functions ARN - retrieved from admin bootstrap SSM
  step_functions_arn = data.aws_ssm_parameter.step_functions_arn.value

  # Account Registry Table - retrieved from admin bootstrap SSM
  account_registry_table_name = data.aws_ssm_parameter.account_registry_table_name.value
...
module "admin_backend" {
  ...
  account_registry_table_name = local.account_registry_table_name
  step_functions_arn         = local.step_functions_arn

  # Step Functions ARNs for account lifecycle management (both use the same state machine)
  create_account_step_function_arn = local.step_functions_arn
  delete_account_step_function_arn = local.step_functions_arn

  # Cross-account access
  admin_account_id        = local.admin_account_id
  account_account_id       = local.account_account_id
...
module "create_infra_worker" {
  ...
  # DynamoDB tables
  account_registry_table_name = local.account_registry_table_name
  account_public_table_name   = local.account_public_table_name
...
# Setup RBAC Worker Lambda (Default RBAC Creation for New Accounts via IMS)
...
# Create Admin Worker Lambda (Account Admin User Creation)
...
# ==============================================
# Sys App Frontend Resources
# ==============================================

# S3 bucket for Sys App Frontend
resource "aws_s3_bucket" "app_frontend" {
  count  = var.enable_app_frontend ? 1 : 0
  bucket = "app-fe-${var.workspace_prefix}-${random_id.bucket_suffix[0].hex}"

  tags = merge(local.common_tags, {
    Name    = "app-fe-${var.workspace_prefix}"
    Purpose = "SysAppFrontend"
  })
}
...
resource "aws_s3_bucket_versioning" "app_frontend" {
  count  = var.enable_app_frontend ? 1 : 0
  bucket = aws_s3_bucket.app_frontend[0].id
...
# Sys App Frontend Web Server Lambda
module "app_frontend_web_server" {
  count  = var.enable_app_frontend ? 1 : 0
  source = "./modules/app-frontend-web-server"

  environment    = var.workspace_prefix
  s3_bucket_name = aws_s3_bucket.app_frontend[0].bucket
  runtime        = var.lambda_runtime
  timeout        = var.lambda_timeout
  memory_size    = var.lambda_memory_size
  common_tags    = local.common_tags
...
module "infrastructure_ssm_outputs" {
  ...
  outputs = {
    ...
    platform-account-id     = var.enable_platform_bootstrap && var.enable_cognito ? module.platform_bootstrap[0].platform_account_id : ""
...
module "ims_service" {
  ...
  account_registry_table_name = local.account_registry_table_name
  account_registry_table_arn  = "arn:aws:dynamodb:${local.region}:${local.account_id}:table/${local.account_registry_table_name}"
...
module "platform_bootstrap" {
  ...
  # DynamoDB configuration (using account registry table)
  rbac_table_name      = local.account_registry_table_name
  ...
  provisioner "local-exec" {
    command = <<-EOT
      aws lambda update-function-configuration \
        ...
        --environment "Variables={NODE_ENV=${var.workspace_prefix},LOG_LEVEL=${var.app_backend_log_level},ACCOUNT_REGISTRY_TABLE_NAME=${local.account_registry_table_name},ADMIN_ACCOUNT_ID=${local.admin_account_id},ACCOUNT_ACCOUNT_ID=${local.account_account_id},CROSS_ACCOUNT_ROLE_NAME=${local.cross_account_role_name},WORKSPACE=${var.workspace_prefix},AWS_REGION_NAME=${var.aws_region},IMS_API_URL=${module.api_gateway[0].api_gateway_url}}" \
        --region ${var.aws_region}
    EOT
  }
