...
class DynamoDBService {
            ...
            logger.info(
                {
                    tenantEntry,
                    tableName:
                        config.DYNAMODB.TENANT_PUBLIC_TABLE || 'TENANT_PUBLIC',
                },
    ...
    async createTenantDynamoDBTable(tenantData) {
        ...
        try {
            ...
            // Naming: account-{tenantId}-admin-private-{workspace}
            const workspace = process.env.WORKSPACE || 'dev';
            const tableName = `account-${tenantData.tenantId}-admin-private-${workspace}`;
    ...
    generateDynamoDBCloudFormationTemplate(tenantId) {
        const workspace = process.env.WORKSPACE || 'dev';
        const tableName = `account-${tenantId}-admin-private-${workspace}`;
